...
To reduce the amount of allocations, the GreenNode is a [<abbr title="dynamically sized type">DST</abbr>](https://doc.rust-lang.org/reference/dynamically-sized-types.html), which uses a single allocation for header and children. Thus, it is only usable behind a pointer.
